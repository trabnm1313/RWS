...
const _SlimeP = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/slimePink.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const SlimeP = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "SlimeP:"+count++,
            Health: 100,
