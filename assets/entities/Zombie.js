...
const _Zombie = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/Zombie.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const Zombie = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "Zombie:"+count++,
            Health: 100,
