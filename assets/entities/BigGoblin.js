...
const _BigGoblin = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/BigGoblin.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const BigGoblin = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "BigGoblin:"+count++,
            Health: 100,
