...
const _SkeletonB = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/Skeleton_Bow.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const SkeletonB = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "SkeletonB:"+count++,
            Health: 100,
