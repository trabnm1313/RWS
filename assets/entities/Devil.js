...
const _Devil = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/Devil.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const Devil = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "Devil:"+count++,
            Health: 100,
