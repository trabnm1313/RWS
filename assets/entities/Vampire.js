...
const _Vampire = (props) => {
    ...
    return(
        <TouchableWithoutFeedback onPress={() => props.engine.current.dispatch(response)}>
            <Image style={{ position: "absolute", width: bodyWidth, height: bodyHeight, left: xBody, top: yBody, borderWidth: 1}} source={require("../images/Monster/Vampire.gif")}></Image>
        </TouchableWithoutFeedback>
    )
}

const Vampire = (engine, pos, size, status) => {
    if(status == null){
        status = {
            id: "Vampire:"+count++,
            Health: 100,
